    ...
    <el-dialog title="Change Password" :visible.sync="dialogVisible" width="30%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="user" label-width="80px" style="padding-right: 20px" :rules="rules" ref="formRef">
        <el-form-item label="Initial Password" prop="password">
          <el-input show-password v-model="user.password" placeholder="Initial Password"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    update() {
      ...
      this.$request.put('/user/update', this.user).then(res => {
        if (res.code === '200') {
          ...
          // Triggers the parent's data update
    ...
    handleAvatarSuccess(response, file, fileList) {
      // Replace the user profile picture with a link to the uploaded image
